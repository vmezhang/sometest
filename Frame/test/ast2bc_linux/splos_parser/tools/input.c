...
#include <stdio.h>
	...
	#include "../fs/include/fileinterface.h"
#else
    #include <sys/mman.h>
    #include <sys/types.h>
    #include <sys/stat.h>
...
static char *FileFix[] = {
	...
	#define FILE_FIX(num, fix) fix, 
	#include "include/confg/conf.h"
	#undef FILE_FIX
	NULL
};
...
void ReadSourceFile (char *filename) 
{
    ...
    // 先cp 到fs, 在打开
    From_Host_To_MySFS (lf_filename, filename);
